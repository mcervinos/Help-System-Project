    ...
    <>
      ...
      <main className="bg-gray-900 min-h-screen text-white p-8">
        <div className="bg-gray-800 p-8 rounded-xl shadow-lg max-w-4xl mx-auto w-full">
          <h2 className="text-3xl font-bold text-center mb-6">All Questions</h2>
              ...
              <table className="min-w-full divide-y divide-gray-700 rounded-lg overflow-hidden">
                <thead className="bg-gray-700">
                  ...
                  {["Ticket ID", "Title", "Department", "Status", "Posted By"].map((heading) => {
                    let cornerClass = "";
                    if (heading === "Ticket ID") cornerClass = "rounded-tl-lg";
                    else if (heading === "Posted By") cornerClass = "rounded-tr-lg";
                ...
                <tbody className="bg-gray-800 divide-y divide-gray-700">
                  {faqs
                    .filter((faq) =>
                      ...
                      (!posterFilter || faq.posterName.toLowerCase().includes(posterFilter.toLowerCase()))
                    )
                    .map((faq) => (
                    <tr
                      key={faq.id}
                    ...
                    >
                      <td className="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-100">#{faq.id}</td>
                      <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-300">{faq.title}</td>
                      <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-300">{faq.department}</td>
                      <td className="px-6 py-4 whitespace-nowrap text-sm">
                        <span
                          className={`px-2 inline-flex text-xs leading-5 font-semibold rounded-full ${statusColor[faq.status]}`}
                        >
                          {faq.status}
                        </span>
                      </td>
                      <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-300">{faq.posterName}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
